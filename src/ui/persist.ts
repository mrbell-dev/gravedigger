...
const EMPTY: Stats = { played: 0, wins: 0, gold: 0, silver: 0, bronze: 0, streak: 0, bestStreak: 0 };
...
export function recordResult(status: Status): Stats {
  const st = loadStats();
  st.played += 1;
  if (status.kind === "won") {
    st.wins += 1;
...
/** Remove ?seed= from the address bar so a later refresh resumes normally. */
...
/** A shareable link that starts this exact graveyard. */
export function shareLink(seed: number): string {
  try {
    const url = new URL(location.href);
